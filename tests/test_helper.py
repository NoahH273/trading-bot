...
@pytest.mark.parametrize(
    "request_url,df_schema,safe,raises,expected",
    [
        # Valid cases
        (f"https://api.polygon.io/v2/aggs/ticker/AAPL/range/1/day/2024-01-09/2024-01-12?adjusted=true&sort=asc&limit=120&apiKey={POLYGON_API_KEY}", 
         None, True, None, pl.read_parquet("tests/Test-Data/get_paginated_request.parquet")), # Valid request URL with no schema
        (f"https://api.polygon.io/v2/aggs/ticker/AAPL/range/1/day/2024-01-09/2024-01-12?adjusted=true&sort=asc&limit=120&apiKey={POLYGON_API_KEY}", 
         ["v", "vw", "o", "c", "h", "l", "t", "n"], True,
        ...
        (f"https://api.polygon.io/v1/last/stocks/INVALID?apiKey={POLYGON_API_KEY}", 
         None, True, ValueError, None), # Invalid ticker
        (f"https://api.polygon.io/v1/last/stocks/AAPL?apiKey={POLYGON_API_KEY}", 
         {"ticker": str, "last": float}, True,
         ValueError, None) # Invalid schema for the response
    ]
)
def test_get_paginated_request(request_url, df_schema, safe, raises, expected):
    if raises:
